...
| `USER_AGENT` | нет | — | Разрешить запросы только с этого User-Agent |
| `HASH_LENGTH` | нет | `12` | Длина хеша в символах |
| `MAX_FILE_SIZE_MB` | нет | `10` | Максимальный размер файла в МБ |
| `RESIZE_IMAGES` | нет | `true` | Генерировать OG (1200x630) и RE (1920x1080) превью |
| `SHARPEN` | нет | `false` | Применять sharpen после ресайза |
...
### Получить изображение / превью

```
GET /<hash>.png       — оригинал PNG (полный размер)
GET /<hash>.og.png     — превью 1200×630 для OG-метатегов
GET /<hash>.re.png     — превью 1920×1080 для страницы
```

Превью генерируются автоматически при загрузке, если `RESIZE_IMAGES=true`.  
Если изображение меньше целевого размера — ресайз не применяется (сохраняется исходное разрешение).
...
В `<img>` на странице вставляется превью 1920×1080.
